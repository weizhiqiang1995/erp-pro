...
layui.define(['layer', "fsCommon"], function(exports) {
	var layer = layui.layer,
		...
		FsTree = function() {
			this.config = {
				...
				beforeRename: undefined,//节点重命名前的回调函数
				onRename: undefined,//节点重命名后的回调函数
				beforeDrag: undefined,//节点拖拽前的回调函数
				beforeDrop: undefined,//节点拖拽后的回调函数
				enable: true,//异步还是同步加载，默认同步
				checkEnable: false,//是否显示复选框,默认为否
				dragEnable: false,//是否可拖拽,默认为否
				...
				showLine: true,
				showIcon: true,
				onAsyncSuccess: null,//异步加载完之后的函数
				chkStyle: 'checkbox',
				beforeCheck: undefined,//选中之前  判断
				onCheck: undefined,//选中回调函数
				expandSpeed: '',//zTree 节点展开、折叠时的动画速度，设置方法同 JQuery 动画效果中 speed 参数。三种预定速度之一的字符串("slow", "normal", or "fast")
			}
		};
	...
	FsTree.prototype.showTree = function(data) {
		...
		var setting = {
			view: {
				...
				expandSpeed: _this.config.expandSpeed,//不要展开、折叠时的动画
			},
			data: {
				key: {
			...
			callback: {
				...
				onAsyncSuccess: function() {
					...
					if(typeof(_this.config.onAsyncSuccess) == "function") {
	 ...
	 */
	FsTree.prototype.refresh = function(tid, callback) {
		var _this = this;
		var zTree = $.fn.zTree.getZTreeObj(_this.config.id),
			type = "refresh",
			silent = false;
