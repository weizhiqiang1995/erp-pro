...
(function(a) {
    a.fn.upload = function(D) {
        var options = {
            "action":"/upload",
            "data-num":"10",
            "data-type":"png,jpg,jpeg,gif,zip,pdf,doc,docx,rar",
            // "data-height":"992",
            "data-width":"1920",
            "data-value":"",
            "data-size":1024*100,
        ...
        if (B.value) {
            ...
            var f = 0;
            var completeUrl = ""
            b.attr("oldurl", B.value);
            for (var x in p) {
                if (s < B.num && p[s]) {
                    var ty = p[s].toLowerCase().split(".").splice(-1).join();
                    if (["png", "jpg", "jpeg", "gif", "bmp"].indexOf(ty) >= 0) {
                    	if(options.uploadType == '1'){
                    		h = a("<li class='item success'></li>").append(g).append(typeof D == "function" ? E : "").data("url", fileBasePath + "/images/upload/smpropic/" + p[s]).attr("data-filename", c(p[s])).css("background-image", "url('" + fileBasePath + "/images/upload/smpropic/" + p[s] + "')").insertBefore(A);
                    	}else{
                    		h = a("<li class='item success'></li>").append(g).append(typeof D == "function" ? E : "").data("url", p[s]).attr("data-filename", c(p[s])).css("background-image", "url('" + fileBasePath + p[s] + "')").insertBefore(A);
                    	}
                		h.attr("oldurl", p[s]);
                    }else{
                        h = $("<li class=\"item success\" data-filename=\"YPCn1FUBacGJOuYKb6\">"+
                            k(ty)+
                            '<div class="filename">'+ty+'</div>'+
                            "</li>").insertBefore(A);
                    }
                    r.data("num", ++f).removeClass("empty");
                    q();
                    if(options.uploadType == '1'){
                    	completeUrl += fileBasePath + "\\images\\upload\\smpropic\\" + p[s] + ",";
                    }else{
                    ...
                    var i = l(G[F], function(K, J, I) {
                        ...
                        a.ajax({
                            ...
                            success: function(M) {
                                ...
                                if (M.returnCode == 0) {
                                	if(M.bean.type == '1'){
                                		J.addClass("success").append(g).append(typeof D == "function" ? E : "").data("url", fileBasePath + "\\images\\upload\\smpropic\\" + M.bean.picUrl);
                                		J.attr("oldurl", M.bean.picUrl);
                                	}else{
                                		J.addClass("success").append(g).append(typeof D == "function" ? E : "").data("url", fileBasePath + M.bean.picUrl);
                                		J.attr("oldurl", M.bean.picUrl);
                                	}
                                } else {
                                    J.addClass("error").attr("data-error", M.returnMessage ? M.returnMessage : "服务端返回数据异常")
                                }
                                q()
                            },
                            xhr: function() {
        ...
        function l(G, L) {
            ...
            var F = G.size,
                ...
                i = a(w);
            var I = B.type.split(",");
            var J1 = J.toUpperCase();
            var J2 = J.toLowerCase();
            if (I.indexOf(J1) < 0 && I.indexOf(J2) < 0) {
                //var K = "不能上传." + J + "的文件!"
                var K = "不支持"+J+"格式";
            }
            if (["png", "jpg", "jpeg", "gif", "bmp"].indexOf(J) < 0 && F > B.size * 1024) {
                // var K = "不能上传大于." + B.size + "KB 的文件!"
                var K = "文件最大" + B.size + "KB"
            }
            ...
            H.onload = function() {
                ...
                if (r.children('li[data-filename="' + P + '"]').length > 0) {
                    return false
                }
                ...
                if (["png", "jpg", "jpeg", "gif", "bmp"].indexOf(J) >= 0) {
                    ...
                    O.onload = function() {
                        ...
                        aa.fillStyle = "#fff";
                        aa.fillRect(0, 0, Q.width, Q.height);
                    ...
                    var N = k(J);
                    i.append(N).append("<div class='filename'>" + J + "</div>").append(e);
                    var M = G;
                    L(M, i, G);
                    return
                }
        ...
        function q() {
            ...
            r.children("li.success").each(function() {
                ...
                qw.push(a(this).attr("oldurl"))
            });
            if (r.children("li").not(".add").length >= B.num && B.num > 1) {
                A.hide()
            } else {
                A.show()
            }
            if (r.children("li").not(".add").length == 0) {
                r.addClass("empty")
            }
            b.val(i.join(","));
            b.attr("oldurl", qw.join(","));
        }
        function z(i) {
