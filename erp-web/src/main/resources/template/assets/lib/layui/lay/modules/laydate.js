  ...
  Class.prototype.list = function(type, index){
    ...
    ,ul = lay.elem('ul', {
      'class': ELEM_LIST + ' ' + ({
        ...
        ,timeminute: 'laydate-timeminute-list'
      })[type]
    })
    ...
    if(type === 'year'){ //年列表
      ...
      setTimeStatus();
          ...
          lay(ul).find('ol').each(function(i, ol){
            lay(ol).find('li').each(function(ii, li){
              that.limit(lay(li), [{
                ...
                hours: that[startEnd].hours
                ,minutes: ii
              }][i], index, [['hours'], ['hours', 'minutes']][i]);
            });
          });
          if(!options.range) that.limit(lay(that.footer).find(ELEM_CONFIRM), that[startEnd], 0, ['hours', 'minutes']);
        };
        if(options.range){
          if(!that[startEnd]) that[startEnd] = {
            hours: 0
        ...
        lay.each([24, 60], function(i, item){
          var li = lay.elem('li'), childUL = ['<p>'+ lang.time[i] +'</p><ol>'];
          if(item == '60' && !isNull(options.minutesinterval)){
        	  item = 60 / options.minutesinterval;
        	  var _NowMinutes = new Date(getFormatDate()).format("mm");
        	  lay.each(new Array(item), function(ii){
        		  if(options.minutesinterval * ii > _NowMinutes){
        			  childUL.push('<li>'+ options.minutesinterval * ii +'</li>');
        		  }else{
        			  childUL.push('<li'+ (that[startEnd][hms[i]] === ii ? ' class="'+ THIS +'"' : '') +'>'+ options.minutesinterval * ii +'</li>');
        		  }
        	  });
          }else{
        	  lay.each(new Array(item), function(ii){
