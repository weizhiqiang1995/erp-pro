...
var sysMessageMainMation = '';//系统基础信息json
var addrs = '';//地址json

//系统cookies从哪里获取， true：从用户登陆获取；false：跨域获取
var getMessageCookiesByUrl = true;

if(!getMessageCookiesByUrl){//跨域获取
	if(isNull(getCookie("userToken"))){
		if(isNull(GetUrlParam("userToken"))){//如果url后面没有跟usertoken，返回404页面
			location.href = reqBasePath + '/tpl/sysmessage/500.html';
		}else{
...
window.addEventListener('message',function(e){
	...
	if(isNull(authList)){
		localStorage.setItem("authpoints", e.data);
	}
});
...
//系统基础信息
if(isNull(getCookie("sysMessageMainMation"))){
	MiniSite.JsLoader.load(reqBasePath + "json/main.js", function(){
		setCookie('sysMessageMainMation', JSON.stringify(sysMessageMainMation), 'd30');
	});
}else{
	sysMessageMainMation = JSON.parse(unescape(getCookie("sysMessageMainMation")));
}

webSocketPath = sysMessageMainMation.webSocketPath;

//地址
MiniSite.JsLoader.load(reqBasePath + "json/city.js", function(){
});
...
//		win.console && console.error && console.error('Layui hint: ' + msg);
	...
	modules = {// 内置模块
	    ...
	    jqueryUI: 'modules/jqueryui/jquery-ui-1.10.3.custom',//jQuery拖拽
