        ...
        var windowIndex = layer.open({
            ...
            //销毁回调
            	...
            	if(typeof(options.cancel) == "function") {
        			options.cancel(windowDom, index);
        		}
            },
            //打开回调
    ...
    var common = {
        ...
        addTaskItem: function (id, title, iconTitle, loadBottomMenuIcon) {
        	...
        	if(loadBottomMenuIcon === '1'){//只展示图标
        		...
        		taskItem = $('<li win-id="' + id + '" class="winui-task-item">' + iconTitle + title + '</li>');
        	}
            $('.winui-taskbar-task:visible').append(taskItem);
            return taskItem;
        },
        //重置元素的MouseUp事件
...
//                });
    ...
    var call = {
        ...
        taskItemMouseUp: function (e) {
            ...
            if (e.button == 2) {
                ...
                var div = '<ul class="task-contextmenu" style="' + style + 'width:auto">';
    ...
    //打开个人中心
    winLayer.openSysPersonal = function (loadBottomMenuIcon) {
        var that = this;
        $.get(winui.path + '../../../tpl/syspersonal/syspersonal.html', {}, function (content) {
        	var title = '<i class="fa title-icon fa-fw fa-user" style="background-color: #0491fe;color: #ecf3f8;"></i>' + '<font class="win-title-class">个人中心</font>';
            var iconTitle = '<i class="fa title-icon-big fa-fw fa-user" style="background-color: #0491fe;color: #ecf3f8;"></i>';
        	that.open({
