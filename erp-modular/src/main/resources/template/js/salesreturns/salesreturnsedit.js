...
layui.config({
	...
	layui.use(['form', 'tagEditor'], function(form) {
		...
		function initDataShow(){
			AjaxPostUtil.request({url: reqBasePath + "salesreturns003", params: {rowId: parent.rowId}, type: 'json', callback: function(json) {
				if(json.returnCode == 0) {
					if(isNull(json.bean)){
						$("#showForm").html("");
		...
		function loadTockByDepotAndMUnit(rowNum){
			...
			if(!isNull(chooseDepotId) && !isNull(chooseUnitId)){
				...
				if(inTockObject < 0){
					//获取库存
					AjaxPostUtil.request({url: reqBasePath + "material011", params: {depotId: chooseDepotId, mUnitId: chooseUnitId, rowId: parent.rowId}, type: 'json', callback: function(json) {
						if(json.returnCode == 0) {
							var currentTock = 0;
							if(!isNull(json.bean)){
		...
		form.on('submit(formEditBean)', function(data) {
			...
			if(winui.verifyForm(data.elem)) {
				...
				AjaxPostUtil.request({url: reqBasePath + "salesreturns004", params: params, type: 'json', callback: function(json) {
					if(json.returnCode == 0) {
						parent.layer.close(index);
