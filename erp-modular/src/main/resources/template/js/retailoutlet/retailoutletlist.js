...
layui.config({
    ...
    function deletemember(data){
        layer.confirm('确认要删除信息吗？', { icon: 3, title: '删除操作' }, function (index) {
            AjaxPostUtil.request({url:reqBasePath + "delcommon003", params: {rowId: data.id}, type:'json', callback:function(json){
    ...
    $("body").on("click", "#downloadExcel", function () {
    	...
    	postDownLoadFile({
			url : reqBasePath + 'retailoutlet005?userToken=' + getCookie('userToken') + '&loginPCIp=' + returnCitySN["cip"],
