...
layui.config({
    ...
    table.render({
        ...
        elem: '#messageTable',
        method: 'post',
        url: reqBasePath + 'receivables001',
        ...
        cols: [[
            ...
            { field: 'supplierName', title: '付款单位', align: 'left', width: 150},
    ...
    function edit(data){
        rowId = data.id;
        _openNewWindows({
            url: "../../tpl/receivables/receivablesedit.html",
            title: "编辑",
            pageId: "receivablesedit",
            area: ['90vw', '90vh'],
    ...
    //删除
    function deletereceivables(data){
        layer.confirm('确认要删除信息吗？', { icon: 3, title: '删除操作' }, function (index) {
            AjaxPostUtil.request({url:reqBasePath + "receivables005", params: {rowId: data.id}, type:'json', callback:function(json){
    ...
    function details(data){
        rowId = data.id;
        _openNewWindows({
            url: "../../tpl/receivables/receivablesinfo.html",
            title: "详情",
            pageId: "receivablesinfo",
            area: ['90vw', '90vh'],
            callBack: function(refreshCode){
                if (refreshCode == '0') {
                    winui.window.msg("操作成功", {icon: 1,time: 2000});
    ...
    $("body").on("click", "#addBean", function(){
        _openNewWindows({
            url: "../../tpl/receivables/receivablesadd.html",
            title: "新增",
            pageId: "receivablesadd",
            area: ['90vw', '90vh'],
            callBack: function(refreshCode){
                if (refreshCode == '0') {
                    winui.window.msg("操作成功", {icon: 1,time: 2000});
