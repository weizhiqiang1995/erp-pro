...
layui.config({
    ...
    var $ = layui.$,
        ...
        table = layui.table;
    authBtn('1571811377375');//新增
    authBtn('1572318610899');//导出
    ...
    table.render({
        ...
        elem: '#messageTable',
        method: 'post',
        url: reqBasePath + 'advancecharge001',
        ...
        cols: [[
            ...
            { field: 'supplierName', title: '付款会员', align: 'left', width: 150},
    ...
    function edit(data){
        rowId = data.id;
        _openNewWindows({
            url: "../../tpl/advancecharge/advancechargeedit.html",
            title: "编辑",
            pageId: "advancechargeedit",
            area: ['90vw', '90vh'],
    ...
    //删除
    function deleteadvancecharge(data){
        layer.confirm('确认要删除信息吗？', { icon: 3, title: '删除操作' }, function (index) {
            AjaxPostUtil.request({url:reqBasePath + "advancecharge005", params: {rowId: data.id}, type:'json', callback:function(json){
    ...
    function details(data){
        rowId = data.id;
        _openNewWindows({
            url: "../../tpl/advancecharge/advancechargeinfo.html",
            title: "详情",
            pageId: "advancechargeinfo",
            area: ['90vw', '90vh'],
            callBack: function(refreshCode){
                if (refreshCode == '0') {
                    winui.window.msg("操作成功", {icon: 1,time: 2000});
    ...
    $("body").on("click", "#addBean", function(){
        _openNewWindows({
            url: "../../tpl/advancecharge/advancechargeadd.html",
            title: "新增",
            pageId: "advancechargeadd",
            area: ['90vw', '90vh'],
            callBack: function(refreshCode){
                if (refreshCode == '0') {
                    winui.window.msg("操作成功", {icon: 1,time: 2000});
    ...
    $("body").on("click", "#downloadExcel", function () {
    	...
    	postDownLoadFile({
			url : reqBasePath + 'advancecharge007?userToken=' + getCookie('userToken') + '&loginPCIp=' + returnCitySN["cip"],
			params: {billNo: $("#billNo").val(), startTime: startTime, endTime: endTime},
			method : 'post'
		});
    });
    
    exports('advancechargelist', {});
});
