...
layui.config({
	...
	layui.use(['form'], function(form) {
		...
		function initDataShow(){
			AjaxPostUtil.request({url: reqBasePath + "purchaseout003", params: {rowId: parent.rowId}, type: 'json', callback: function(json) {
				if(json.returnCode == 0) {
					if(isNull(json.bean)){
						$("#showForm").html("");
		...
		form.on('submit(formEditBean)', function(data) {
			...
			if(winui.verifyForm(data.elem)) {
				...
				AjaxPostUtil.request({url: reqBasePath + "purchaseout004", params: params, type: 'json', callback: function(json) {
					if(json.returnCode == 0) {
						parent.layer.close(index);
