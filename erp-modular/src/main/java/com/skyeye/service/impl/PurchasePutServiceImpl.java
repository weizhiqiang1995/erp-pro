...
public class PurchasePutServiceImpl implements PurchasePutService{
	...
	public void insertPurchasePutMation(InputObject inputObject, OutputObject outputObject) throws Exception {
		...
		if(ToolUtil.isJson(depotheadStr) && ToolUtil.isJson(otherMoneyList)){
			...
			purchasePutDao.insertPurchasePutMation(depothead);
			purchasePutDao.insertPurchasePutChildMation(entitys);
		}else{
	...
	public void queryPurchasePutMationToEditById(InputObject inputObject, OutputObject outputObject) throws Exception {
		...
		//获取主表信息
		Map<String, Object> bean = purchasePutDao.queryPurchasePutMationToEditById(map);
		if(bean != null && !bean.isEmpty()){
			//获取子表信息
			List<Map<String, Object>> norms = purchasePutDao.queryPurchasePutItemMationToEditById(bean);
	...
	public void editPurchasePutMationById(InputObject inputObject, OutputObject outputObject) throws Exception {
		...
		if(ToolUtil.isJson(depotheadStr) && ToolUtil.isJson(otherMoneyList)){
			...
			depothead.put("tenantId", tenantId);
			//删除之前绑定的产品
			purchasePutDao.deletePurchasePutChildMation(map);
			//重新添加
			purchasePutDao.editPurchasePutMationById(depothead);
			purchasePutDao.insertPurchasePutChildMation(entitys);
		}else{
			outputObject.setreturnMessage("数据格式错误");
		}
	...
	public void queryMationToExcel(InputObject inputObject, OutputObject outputObject) throws Exception {
		Map<String, Object> params = inputObject.getParams();
        params.put("tenantId", inputObject.getLogParams().get("tenantId"));
        List<Map<String, Object>> beans = purchasePutDao.queryMationToExcel(params);
        ...
        //采购入库单信息导出
        ExcelUtil.createWorkBook("采购入库单", "采购入库单详细", beans, key, column, dataType, inputObject.getResponse()); 
	}
